...
func (saleReq *SaleRequest) GetTotalProductQty(id string, db *gorm.DB) (error, int) {
	...
	switch t := saleReq.Product.(type) {
	case *Glass:
		err := db.Model(&t).Where("id = ?", id).Find(&p).Error
		if err != nil {
			return err, 0
		}
	...
	return fmt.Errorf("Internal error"), 0
...
func (saleReq *SaleRequest) IncreaseQty(id string, qty int, db *gorm.DB) error {
	...
	case *Glass:
		...
		err = db.Model(&t).Where("id = ?", id).Update("quantity", newQty).Error
		if err != nil {
			return err
		}
